...
- **公平并发控制**：
  ...
  - 长文本分段合成按分段粒度申请与释放并发许可，防止单个超长请求长时间独占所有执行配额。
  - 客户端提前断开连接时，立即中止已排队请求与上游正在合成的流。
- **Web 工作台**：
  - React 单页应用，具备双模式音频播放：优先使用 `MediaSource` 渐进式流式播放，不支持时平滑降级为 Blob 对象 URL。
  - 音色检索与地区过滤、收藏夹置顶保存。
  - 本地 UTF-8 `.txt` 文件导入（浏览器本地读取，上限 256 KiB / 20,000 码点，绝不向云端上传或持久化文件内容）。
...
- **安全与隐私**：
  - 绝不记录合成文本日志，无任何外部数据上报；文本与认证凭据不在磁盘落地。
...
- **真实流式遥测**：界面展示真实的合成状态（请求中状态、流式播放状态、服务度计划分段数，以及实际已接收音频字节数）。edgeTTS 不展示虚假的百分比进度条、预估剩余时间 (ETA) 或已完成段落计数器。
...
- **并发合成限制**：默认最多 4 个活跃合成流，16 个排队请求。长文本合成按分段动态申请释放并发许可。队列满时返回 HTTP 503。
...
- **平滑停机机制**：30 秒停机缓冲（`--stop-timeout 30`），保证正在流式传输的语音正常完成。
