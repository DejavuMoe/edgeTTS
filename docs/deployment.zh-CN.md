...
| 镜像标签                                    | 描述                               | 适用场景                 |
| :------------------------------------------ | :--------------------------------- | :----------------------- |
| `ghcr.io/dejavumoe/edgetts:0.5.0`           | 严格语义化版本发布标签             | 生产环境常规部署         |
...
先将 `compose.yaml` 中的 `image:` 改成要部署的已发布版本或摘要。`docker compose pull` 只拉取配置指定的镜像，不会把固定的 `0.5.0` 自动升级到其他版本。保留 `.env` 和上一版本镜像引用。
