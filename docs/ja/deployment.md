...
| タグ                                        | 説明                                         | 推奨用途                     |
| :------------------------------------------ | :------------------------------------------- | :--------------------------- |
| `ghcr.io/dejavumoe/edgetts:0.8.0`           | 厳格な SemVer リリースタグ                   | 本番環境の通常デプロイ       |
...
最初に `compose.yaml` の `image:` を目的の公開バージョンまたはダイジェストへ変更します。`docker compose pull` は設定された参照だけを取得し、固定された `0.8.0` を別バージョンへ自動更新しません。`.env` と以前のイメージ参照を保持してください。
