...
4. **Ancestry Verification**: Release tags must target a commit already present in `origin/main` commit history (`git merge-base --is-ancestor`). Tags pointing to unmerged topic branches or detached commits are rejected before compilation.
5. **Rejection of Moved Tags**: The publishing workflow checks `github.event.before`. If an existing tag was moved or force-updated to a new commit (`github.event.before != 000...000`), the workflow immediately fails.
