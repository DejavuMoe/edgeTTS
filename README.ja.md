...
現在の安定版リリース：[v0.1.0](https://github.com/DejavuMoe/edgeTTS/releases/tag/v0.1.0)
...
- **公平な同時実行制御**：
  ...
  - 長文分割合成ではセグメント単位で実行許可の取得・解放を行い、1 つの巨大なリクエストがサーバーリソースを独占することを防止。
...
- **Web ワークベンチ**：
  ...
  - ローカル UTF-8 `.txt` ファイルのインポート（ブラウザローカルで読み込み、上限 256 KiB / 20,000 コードポイント。クラウドへのアップロードやディスク保存は一切行いません）。
...
- **セキュリティとプライバシー**：
  - 合成テキストのログ出力や外部テレメトリ送信は一切行いません。入力テキストや認証情報はディスクに保存されません。
...
| 参照形式                                                                                            | 用途                                                  | 変更可能性                         |
| --------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ---------------------------------- |
| `ghcr.io/dejavumoe/edgetts:0.1.0`                                                                   | 通常の安定版本番デプロイに推奨                        | リリースタグ                       |
| `ghcr.io/dejavumoe/edgetts@sha256:4dca280e9a2dfdaa3ef4b30e1f5d136ad32975ee1ef1763fc8f869e159502034` | 厳格な再現性を保証するイミュータブル指定              | コンテンツアドレス指定（完全不変） |
...
- **同時実行制限**：同時に実行できるストリーム数は最大 4、待機キューは最大 16（長文合成時はセグメントごとに許可を取得・解放、キュー満杯時は HTTP 503）。
